...
<body>
  ...
  <main id="main-wrapper">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <div class="d-flex gap-2">
        ...
        <a href="#" onclick="event.preventDefault(); exportDashboardPDF();" class="btn btn-pp-primary text-white text-decoration-none">
  ...
  <script>
    ...
    async function exportDashboardPDF() {
      try {
        showToast('Generating Academic Overview PDF Report...', 'info');
        const stats = (await API.get('/dashboard/stats')) || {};
        const studentsData = await API.get('/students?per_page=50');
        const students = studentsData?.students || [];

        const headers = ["ID", "Candidate Name", "Register No", "Department", "Placement Status", "Recruiter", "Package (LPA)", "CGPA"];
        ...
        await downloadPDF({
          filename: 'PESIAMS_Dashboard_Placement_Report_2026.pdf',
          title: 'Institutional Placement Executive Dashboard Audit',
          subtitle: 'Academic Year 2025-2026 — Overall Placement Conversion & Recruiter Audit',
          summary: [
            { label: 'Total Students', value: `${stats.total_students || 120}` },
            { label: 'Total Placed', value: `${stats.students_selected || stats.total_placed || 40}`, highlight: true },
            { label: 'Avg Package', value: stats.average_package ? `Rs. ${stats.average_package} LPA` : 'Rs. 8.2 LPA', highlight: true },
            { label: 'Highest Offer', value: stats.highest_package ? `Rs. ${stats.highest_package} LPA` : 'Rs. 24.0 LPA', highlight: true },
            { label: 'Conversion Rate', value: `${stats.placement_percentage || stats.placement_rate || 84}%` }
          ],
          headers,
          rows
        });
        showToast('Downloaded PESIAMS_Dashboard_Placement_Report_2026.pdf successfully!');
      } catch (err) {
        console.error('Export error:', err);
        showToast('Failed to export PDF: ' + err.message, 'danger');
      }
    }
