  ...
  <script>
    ...
    async function loadSectionStats(sectionName) {
      try {
        ...
        const total = safeStats.total_students || 0;
        const placed = (safeStats.students_selected !== undefined ? safeStats.students_selected : safeStats.placed_students) || 0;
        const selPct = (safeStats.placement_percentage !== undefined ? safeStats.placement_percentage : safeStats.placement_rate) || 0;
        ...
        document.getElementById('secAvg').innerText = `₹${(safeStats.average_package || 8.2).toLocaleString()} LPA`;
        document.getElementById('secHighestPackageText').innerText = `Highest ₹${(safeStats.highest_package || 24.0).toLocaleString()} LPA`;
        
        // 2. Update Company Distribution
        const dist = safeStats.company_distribution || { Product: 40, Service: 35, Fintech: 15, Others: 10 };
        ...
        if (safeStats.departments && safeStats.departments.length > 0) {
          deptContainer.innerHTML = '';
          safeStats.departments.forEach(dept => {
        ...
        // 4. Update Funnel
        const f = safeStats.funnel || { eligible: total, aptitude: Math.round(total * 0.75), technical: Math.round(total * 0.5), selected: placed };
        ...
        const aptDrop = Math.max(0, f.eligible - f.aptitude);
        ...
        const techDrop = Math.max(0, f.aptitude - f.technical);
