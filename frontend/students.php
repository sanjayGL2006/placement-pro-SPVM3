...
<body>
  ...
  <script>
    ...
    async function loadStudents(page = 1) {
      ...
      if (tbody) {
        tbody.innerHTML = Array(5).fill(0).map(() => `
          <tr>
            <td style="padding-left: 1.5rem;"><div class="placeholder-glow"><span class="placeholder col-6"></span></div></td>
            ...
            <td class="text-end"><div class="placeholder-glow"><span class="placeholder col-10"></span></div></td>
          </tr>
        `).join('');
      }
      ...
      document.getElementById('studentTableBody').innerHTML = list.map(s => {
        ...
        return `
          <tr>
            ...
            <td>${statusBadge}</td>
            <td class="text-end">
              <button class="btn btn-sm btn-pp-outline py-1 px-2" onclick="openViewModal(${s.id})"><i class="fa-regular fa-eye me-1"></i> View</button>
              <a href="documents.html?id=${s.id}" class="btn btn-sm btn-outline-info py-1 px-2 ms-1"><i class="fa-solid fa-folder-open"></i> Docs</a>
            ...
            <div class="pp-card h-100 position-relative">
              ...
              <div class="d-flex justify-content-between align-items-center pt-2 border-top">
                ...
                <div class="d-flex gap-1">
                  <button class="btn btn-sm btn-pp-outline py-1 px-2" onclick="openViewModal(${s.id})"><i class="fa-regular fa-eye"></i> View</button>
                  <a href="documents.html?id=${s.id}" class="btn btn-sm btn-outline-info py-1 px-2"><i class="fa-solid fa-folder-open"></i> Docs</a>
    ...
    async function saveNewStudent(e) {
      ...
      const payload = {
        ...
        register_number: document.getElementById('addStudentReg').value.trim(),
        department_name: document.getElementById('addStudentDept').value,
    ...
    async function openViewModal(studentId) {
      ...
      modalBody.innerHTML = `<div class="text-center py-4"><i class="fa-solid fa-spinner fa-spin me-2" style="font-size: 1.5rem;"></i> Loading profile...</div>`;
      ...
      try {
        ...
        const statusBadge = s.placement_status === 'selected' || s.placement_status === 'joined' || s.placement_status === 'placed'
          ? `<span class="badge-pill-success"><i class="fa-solid fa-circle me-1" style="font-size: 0.4rem;"></i> Placed</span>`
          : (s.placement_status === 'applied' || s.placement_status === 'in-process'
        ...
        modalBody.innerHTML = `
          <div class="d-flex align-items-center gap-3 p-3 bg-light rounded-3 mb-4">
            ...
            <div>
              <h4 class="font-weight-700 mb-1 text-dark">${s.name}</h4>
              <div class="text-muted small mb-2"><i class="fa-solid fa-hashtag me-1"></i> ${s.register_number || 'N/A'}</div>
              ${statusBadge}
            </div>
          </div>
          ...
          <div class="row g-3">
            <div class="col-md-6">
              <div class="p-3 border rounded-3 bg-white">
                ...
                <div class="mb-2"><strong>Academic Batch:</strong> ${s.academic_year || '2023-2026'}</div>
                <div class="mb-2"><strong>CGPA:</strong> ${s.cgpa != null ? s.cgpa : '8.0'}</div>
                <div><strong>Active Backlogs:</strong> ${s.backlogs != null ? s.backlogs : 0}</div>
              </div>
            </div>
            <div class="col-md-6">
              <div class="p-3 border rounded-3 bg-white">
        ...
        modalBody.innerHTML = `<div class="text-danger p-3 text-center"><i class="fa-solid fa-circle-exclamation me-2"></i> ${err.message}</div>`;
      }
    }
    ...
    async function deleteSingleStudent(studentId, studentName) {
      ...
      const res = await Swal.fire({
        ...
        text: `Are you sure you want to delete ${displayTitle}? This action cannot be undone.`,
        icon: 'warning',
    ...
    async function deleteFromEditModal() {
      ...
      const res = await Swal.fire({
        title: 'Delete Student Record?',
        text: `Are you sure you want to delete record for "${studentName || 'Student'}"?`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#EF4444',
        cancelButtonColor: '#6B7280',
    ...
    async function triggerBulkDelete() {
      ...
      const res = await Swal.fire({
        ...
        text: `Are you sure you want to delete ${studentIds.length} selected student records? This action is irreversible.`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#EF4444',
        cancelButtonColor: '#6B7280',
        confirmButtonText: `Delete ${studentIds.length} Records`
      });
  ...
  <div class="modal fade" id="viewStudentModal" tabindex="-1" aria-labelledby="viewStudentModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
      <div class="modal-content border-0 shadow-lg" style="border-radius: 16px;">
        <div class="modal-header border-0 pb-0 px-4 pt-4">
          <h5 class="modal-title font-weight-bold" id="viewStudentModalLabel"><i class="fa-solid fa-id-card text-primary me-2"></i> Student Profile</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body p-4" id="viewStudentModalBody">
          <div class="text-center py-4"><i class="fa-solid fa-spinner fa-spin me-2" style="font-size: 1.5rem;"></i> Loading profile...</div>
        </div>
